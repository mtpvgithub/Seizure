...
public class MainActivity extends Activity implements LocationListener {
	...
	public class Async_task_login extends AsyncTask<Void, Void, String> {
		...
		protected String doInBackground(Void... params) {
        	...
        	Log.i("getLocation() called on MainActivity Login","yes" );
            
            ServiceHelper.login(userName, passWord, IMEI, ""+ latitude, "" + longitude);
			return null;
		}
		...
		protected void onPostExecute(String result) {
			...
			if (ServiceHelper.login_response.toString().trim()==null || ServiceHelper.login_response.toString().trim().equals("0")) {
				showToast("Login Failed!");
			} else {
				MainActivity.arr_logindetails = ServiceHelper.login_response.split(":");
				for (int i = 0; i < MainActivity.arr_logindetails.length; i++) {
					// Log.i("DETAILS", "" + arr_logindetails[i]);
